            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        $rawPath = $item->imagen_path
                        ?? optional($item->producto)->imagen_path
                        ?? null;

                        if ($rawPath) {
                        if (preg_match('#^https?://#i', $rawPath)) {
                        // Ya es URL absoluta (no toques)
                        $imgUrl = $rawPath;
                        } else {
                        // Quita prefijo 'storage/' si algún registro lo tiene
                        $rawPath = preg_replace('#^/?storage/#', '', $rawPath);
                        // Quita slash inicial por si acaso y arma URL desde /public
                        $imgUrl = asset(ltrim($rawPath, '/')); // => https://navasan.site/images/...
                        }
                        } else {
                        $imgUrl = $placeholder; // fallback
                        }
                        ...
                        <tr>
                            <td>
                                <img src="{{ $imgUrl }}" alt="img" style="width:64px;height:64px;object-fit:cover;border-radius:8px;border:1px solid #eee;">
                            </td>
                            <td>
                                @if($item->orden)
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <img src="{{ $imgUrl }}" alt="img" style="width:32px;height:32px;object-fit:cover;border-radius:4px;border:1px solid #ddd;margin-right:8px;">
